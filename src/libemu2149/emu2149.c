...
mix_stero_output(PSG *psg) 
{
    return (int32_t)(psg->ch_out[0] + psg->ch_out[2] + (psg->ch_out[1] << 16));
}

static inline int16_t 
mix_mono_output(PSG *psg) 
{
    return (int16_t)(psg->ch_out[0] + psg->ch_out[1] + psg->ch_out[2]);
}

int16_t
PSG_mono_calc (PSG * psg)
{
  if (!psg->quality) 
  {
    update_output(psg);
    psg->out = mix_mono_output(psg);
  }
  else
  {
...
int32_t
PSG_stereo_calc (PSG * psg)
{
  if (!psg->quality) 
  {
    update_output(psg);
    psg->out = mix_stero_output(psg);
  }
  else
  {
    /* Simple rate converter (See README for detail). */
